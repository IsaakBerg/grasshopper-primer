...
Geometry collection parameters can pass information about existing geometry from Rhino to Grasshopper. You can find them in the Grasshopper ribbon under “Params”-> “Geometry”.  Think of these parameters as data sources. These sources provide information about new geometry and-or references to existing Rhino geometry. 

Each collection stores data on geometry of the appropriate classification. Examples of classifications include points, lines, and circles. When first placed on the canvas a collection parameter is empty.  The user must specify data to add to this collection. Placing data into the collection in an unexpected arrangement may lead to unintended results. The algorithm author should document those details about the data expected at the inputs.
...
Edits of picked data are possible after right-clicking a geometry collection parameter. Choosing the “Manage collection” menu item opens a dialog. In this dialog, double-click a picked data value to edit it using the keyboard. To edit picked data in an inter-active live mode, use Grasshopper's gumball interface. (See 1.1.3.3.)
